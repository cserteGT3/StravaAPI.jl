...
The source code is generated by OpenAPI.jl, which overwrites the `src` and `docs` folders.
...
To generate the package code follow the steps below:

1. Start docker.
2. Instantiate the project in `generator`: `julia --project=generator -e "using Pkg; Pkg.instantiate()"`
3. Generate the code: `julia --project=generator generator/generate.jl`
